...
class BlenderBIM_DatePicker(bpy.types.Operator):
    ...
    def execute(self, context):
        print(self.target_prop)
        print(self.selected_date)
        helper.set_scene_prop(self.target_prop, self.selected_date)
        return {"FINISHED"}

    def draw(self, context):
        self.selected_date = helper.get_scene_prop("datepicker_selected_date")
        current_date = parser.parse(context.scene.datepicker_display_date, dayfirst=True, fuzzy=True)
    ...
    def invoke(self, context, event):
        # self.target_prop = "BIMWorkScheduleProperties.visualisation_start"
        self.display_date = helper.get_scene_prop(self.target_prop) or helper.canonicalise_time(datetime.now())
        context.scene.datepicker_display_date = self.display_date
        context.scene.datepicker_selected_date = self.display_date
...
class BlenderBIM_DatePickerSetDate(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        context.scene.datepicker_selected_date = self.selected_date
        return {"FINISHED"}
...
class BlenderBIM_RedrawDatePicker(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        context.scene.datepicker_display_date = helper.canonicalise_time(date_to_set)
